...
import { ArrowLeft, Building2, MapPin, User, ChevronRight, CheckCircle2, PhoneCall, Mail, Clock, MessageSquare, Plus, CheckSquare } from 'lucide-react';
...
import { cacheSnapshot, addDocWithFallback, updateDocWithFallback } from '../lib/sync';
...
export default function LeadProfile() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "leads"), (snapshot) => {
      ...
      cacheSnapshot('leads', snapshot.docs);
      setLeads(list);
  ...
  const handleCompleteFollowup = async (id: string) => {
    ...
    if (!item) return;
    try {
      await updateDocWithFallback('followups', id, { completed: !item.completed });
      triggerNotification(`${item.with} follow-up marked as ${!item.completed ? 'completed' : 'pending'}.`);
    } catch {
      triggerNotification("Failed to update follow-up.");
    }
  };

  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "followups"), (snapshot) => {
      const list: FollowupItem[] = [];
      snapshot.forEach((doc) => {
        list.push({ id: doc.id, ...doc.data() } as FollowupItem);
      });
      list.sort((a, b) => a.time.localeCompare(b.time));
      cacheSnapshot('followups', snapshot.docs);
      setFollowups(list);
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleAddFollowup = async (e: React.FormEvent) => {
    ...
    try {
      await addDocWithFallback('followups', {
        with: newWith,
        time: newTime,
        status: 'Scheduled',
        type: 'Meeting',
      ...
      triggerNotification(`New follow-up for ${newWith} scheduled!`);
    } catch {
      triggerNotification("Failed to create follow-up.");
    }
  };

  if (leads.length === 0) {
    return (
      <YStack className={`h-full items-center justify-center ${isLight ? 'text-slate-500' : 'text-white/50'}`}>
        <p className="text-sm font-bold">No leads yet. Create one to get started.</p>
      </YStack>
    );
  }
      ...
      {activeSubTab === 'pipeline' ? (
        ...
        <div className="p-4 space-y-4">
          ...
          {!showAddForm ? (
            ...
            <form onSubmit={handleAddFollowup} className={`rounded-2xl p-4 shadow-xl space-y-3 animate-[slideDown_0.2s_ease-out_forwards] border ${
              ...
              <div className={`flex justify-between items-center border-b pb-2 ${isLight ? 'border-slate-200' : 'border-white/15'}`}>
                ...
                <button type="button" onClick={() => setShowAddForm(false)} className={`text-[10px] ${isLight ? 'text-slate-500 hover:text-slate-800' : 'text-white/50 hover:text-white'}`}>Cancel</button>
              </div>

              <div className="space-y-2 text-xs">
                <div>
                  <label className={`block text-[10px] uppercase font-bold mb-1 ${isLight ? 'text-slate-500' : 'text-white/50'}`}>Contact Name</label>
                  <input type="text" required value={newWith} onChange={(e) => setNewWith(e.target.value)} placeholder="e.g. Dhananjay Mane" className={`w-full rounded-xl px-3 py-2 outline-none transition-all border ${isLight ? 'bg-white border-slate-200 text-slate-800 focus:border-sky-400' : 'bg-white/5 border-white/10 text-white focus:border-sky-400'}`} />
                </div>
                <div>
                  <label className={`block text-[10px] uppercase font-bold mb-1 ${isLight ? 'text-slate-500' : 'text-white/50'}`}>Time & Date</label>
                  <input type="text" required value={newTime} onChange={(e) => setNewTime(e.target.value)} placeholder="e.g. Today at 03:45 PM" className={`w-full rounded-xl px-3 py-2 outline-none transition-all border ${isLight ? 'bg-white border-slate-200 text-slate-800 focus:border-sky-400' : 'bg-white/5 border-white/10 text-white focus:border-sky-400'}`} />
                </div>
                <div>
                  <label className={`block text-[10px] uppercase font-bold mb-1 ${isLight ? 'text-slate-500' : 'text-white/50'}`}>Task Notes</label>
                  <textarea value={newNotes} onChange={(e) => setNewNotes(e.target.value)} placeholder="e.g. Discuss final layout specs..." className={`w-full rounded-xl px-3 py-2 outline-none transition-all h-16 resize-none border ${isLight ? 'bg-white border-slate-200 text-slate-800 focus:border-sky-400' : 'bg-white/5 border-white/10 text-white focus:border-sky-400'}`} />
                </div>
              </div>
              <button type="submit" className="w-full py-2 bg-sky-500 text-white rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer shadow-sm font-sans">Save Scheduled Follow-up</button>
            </form>
          )}
          ...
          <div className="space-y-3.5">
            ...
            {followups.map((fu) => (
              <div key={fu.id} className={`p-4 rounded-2xl border transition-all relative overflow-hidden flex flex-col justify-between ${
                fu.completed ? 'bg-emerald-500/5 border-emerald-500/20 opacity-60' : isLight ? 'bg-white border-slate-200 shadow-lg shadow-slate-200/40' : 'bg-white/10 border-white/10 shadow-lg'
              }`}>
                <div className="flex justify-between items-start gap-2">
                  <div>
                    <span className={`text-[8px] font-bold px-2 py-0.5 rounded border uppercase ${isLight ? 'bg-slate-100 text-sky-600 border-slate-200' : 'bg-white/5 text-sky-300 border-white/10'}`}>{fu.type}</span>
                    <h4 className={`text-sm font-extrabold mt-1.5 ${isLight ? 'text-slate-900' : 'text-white'}`}>{fu.with}</h4>
                  </div>
                  <button onClick={() => handleCompleteFollowup(fu.id)} className={`p-2 rounded-xl border transition-all cursor-pointer ${fu.completed ? 'bg-emerald-500 text-white border-emerald-400/20' : isLight ? 'bg-slate-50 hover:bg-slate-100 text-slate-600 border-slate-200' : 'bg-white/5 hover:bg-white/10 text-white/60 border-white/10'}`}>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  <div className={`p-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-100' : 'bg-white/5 border-white/5'}`}>
                    ...
                    <p className={`text-[11px] leading-relaxed font-medium mt-0.5 ${isLight ? 'text-slate-600' : 'text-white/80'}`}>{fu.notes}</p>
                  </div>
                </div>
              </div>
            ))}
